...
import es.ucm.fdi.tp.practica6.GameClient;
...
public class Main {
	...
	private static ApplicationMode apMode;
	...
	private static void parseArgs(String[] args) {
		...
		cmdLineOptions.addOption(constructModesOption()); // -am or --app-mode
	...
	private static void parseObstaclesOption(CommandLine line) throws ParseException {
		...
		if (obsVal != null) {
			try {
				String[] dimObstacles = obsVal.split("");
				if (dimObstacles.length == 1) {
					dimObs = Integer.parseInt(dimObstacles[0]);
				} else {
	...
	private static Option constructModesOption() {
		String optionInfo = "The mode to use ( ";
		for (ApplicationMode i : ApplicationMode.values()) {
			optionInfo += i.getId();
		}
		optionInfo += "). By default, " + ApplicationMode.NORMAL.getId() + ".";
		Option opt = new Option("am", "app-mode", true, optionInfo);
		opt.setArgName("view identifier");
		return opt;
	}

	private static void parseModesOption(CommandLine line) throws ParseException {
		String modeVal = line.getOptionValue("am", ApplicationMode.NORMAL.getId());
		// mode type
		for (ApplicationMode ap : ApplicationMode.values()) {
			if (modeVal.equals(ap.getId())) {
				apMode = ap;
			}
		}
		if (apMode == null) {
	...
	private static Option constructServerHostOption() {
		String optionInfo = "The server's name to use. By default, 'localhost'.";
	...
	/**
	 * Esta opción solo está disponible para los clientes, si no es cliente, se descarta. Si no se proporciona valor, es "localhost"
	 * @param line
	 * @throws ParseException
	 */
	private static void parseServerHostOption(CommandLine line) throws ParseException {
		...
		if (hostVal != null) {
			hostVal.split("");
			if (apMode.equals(ApplicationMode.CLIENT)) { //Si es modo cliente
				if(hostVal != ""){ // Si tiene algún valor
					serverHost = hostVal;
				}
			} 
		} else {
	...
	private static Option constructServerPortOption() {
		String optionInfo = "The port's number to use. By default, 2000.";
	...
	private static void parseServerPortOption(CommandLine line) throws ParseException {
		...
		if (portVal != null) {
			try {
				...
				throw new ParseException("Invalid number for server: " + portVal);
			}
		} else {
			serverPort = 2000;
		}
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case TicTacToe:
			...
			break;
		case Ataxx:
			if (dimRows != null && dimCols != null && dimObs != null) {
				gameFactory = new AtaxxFactoryExt(dimRows, dimCols, dimObs);
			} else {
				gameFactory = new AtaxxFactoryExt(dimRows,dimCols,dimObs);
			}
			break;
	...
	public static void startGame() {
		...
		case WINDOW:
			c = new Controller(g, pieces);
			if(multiviews){
				//Constructoras para cualquier tipo de juego
				for (Piece p : pieces) {
